...
export class TeamMemberController {
  ...
  async addTeamMembers(@Body() addTeamMembersDto: AddTeamMembersDto) {
    const results =
      await this.teamMemberService.addTeamMembers(addTeamMembersDto); // Pass the entire DTO
    return results;
  }
  ...
  async registerTeamMember(
    ...
    try {
      const user = await this.teamMemberService.registerTeamMember(
        name,
        email,
        password,
      );
  ...
  async getTeamMembers(
    ...
    const teamMembers =
      await this.teamMemberService.getTeamMembersByCompany(companyId);
